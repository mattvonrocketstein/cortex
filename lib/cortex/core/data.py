...
################################################################################
IPY_ARGS          = ['-noconfirm_exit']#, '-rcfile=~/code/smash/smash.rc']
...
# Typical instructions for loading services into the universe
LOAD_LINDA        = [ "load_service", ("_linda",),     {} ]
LOAD_POST_OFFICE  = [ "load_service", ("postoffice",), {} ]
LOAD_TERMINAL     = [ "load_service", ("terminal",),   {} ]


class _standardsession(_session):
    def __init__(self):
        super(_standardsession,self).__init__()
        self.add_instruction(LOAD_LINDA)
        self.add_instruction(LOAD_POST_OFFICE)
